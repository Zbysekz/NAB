...
class HtmcoredistalDetector(AnomalyDetector):
    ...
    def modelRun(self, ts, val):
        ...
        if self.firstStep:
            # distal input is empty, before step 1 is nothing
        ...
        if not self.firstStep:
            ...
            raw = Anomaly.calculateRawAnomaly(activeColumns,
                                                     self.tm.cellsToColumns(self.predictiveCells))
        else:
            raw = 0

        #raw = self.tm.anomaly - can't use this
        temporalAnomaly = raw
        ...
        self.predictiveCells = self.tm.getPredictiveCells()

        if PANDA_VIS_ENABLED:
            self.PandaUpdateData(ts, val, valueBits, dateBits, activeColumns, self.predictiveCells)
